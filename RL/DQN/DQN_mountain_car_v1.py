...
class Dqn():
    def __init__(self):
        ...
        self.memory = np.zeros((MEMORY_CAPACITY, NUM_STATES *2 +2)) #存储前后的状态，动作以及奖励
    ...
    def learn(self):
        ...
        q_eval = self.eval_net(batch_state).gather(1, batch_action)#得到当前Q(s,a)，gather只接受LongTensor的index
        q_next = self.target_net(batch_next_state).detach()#得到Q(s',a')，下面选max
        q_target = batch_reward + GAMMA*q_next.max(1)[0].view(BATCH_SIZE, 1)#公式，view为重构张量维度
