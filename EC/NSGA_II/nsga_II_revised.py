...
Z=np.loadtxt(pre_path, delimiter=",", dtype='int')[:, 1]
...
def pareto_fitness(M, D, DIS):
    ...
    return [-sum_s, sum_distance]  # 都是寻找最小
...
def elitlim(pop, front, crowding):
    ...
    for i in range(popsize):
        if best_s > pop[i]['fitness'][0]:
            best_s = pop[i]['fitness'][0].copy()
            print('best', best_s)
        if best_distance > pop[i]['fitness'][1]:
            best_distance = pop[i]['fitness'][1].copy()
